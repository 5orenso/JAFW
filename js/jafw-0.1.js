	...
	var query_string   = {};
		...
		$(css_selector + ' .' + settings.class_click).bind({
			...
			for (var i=1; i<=5; i++) {
				...
				//var data;
				//if (el.is('input')) {
				//    if (settings.debug >= 3) methods.debug($(this), el + ' is input.', 'warn');
				//    $(el).prop("checked", true);
				//    data = $.extend({}, methods.query_string(param), {'f' : el.attr('name'), 'value' : el.val(), 'checked' : el.checked});
				//} else {
				//    data = methods.query_string(param);
				//}
